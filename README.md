...
PDF's are generated by sending an HTTP GET request to the root directory "/" with a JSON body of the form (if using unregisted files):
```
{
...
```
http://localhost:27182/?tmpl=TEMPALATE_ID&rsc="RESOURCE_ID&rsc="SOME_OTHER_RESOURCE_ID"&dtls="DETAILS_ID"
```

If you provide both a reference to a file and include it in the JSON body, the file you sent in the body will be used.
### Registering a file
Files are registered by sending an HTTP POST request to the root directory "/" with a JSON body of the form:
```
{
